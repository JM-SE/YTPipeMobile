...
1. **Settings and secure config**: Build settings screen, validate `apiBaseUrl` and `mobileApiToken`, persist settings securely, and support health/status test calls.
...
3. **Dashboard MVP**: Display `/status` operational summaries for readiness, polling, email, quota, and channel counts with loading/error/refresh states.
...
5. **Manual actions**: Add explicit sync and poll actions with confirmation, mutation feedback, quota-blocked messaging, and follow-up refreshes.
